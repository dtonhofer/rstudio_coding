...
for file in description.md; do
   ...
   if [[ ! -f $template ]]; then
      ...
      cat "$template" | \
        sed "s~%DIAGRAMS%~${raw_master}/course_2/confidence_interval/diagrams~g" | \
        sed "s~%PLOTS%~${raw_master}/course_2/confidence_interval/plots~g" | \
        sed "s~%CODE%~${blob_master}/course_2/confidence_interval/code~g" \
      > "../$file"
   fi
done
